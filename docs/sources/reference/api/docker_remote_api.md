...
# Docker Remote API
 ...
 - authConfig, set as the `X-Registry-Auth` header, is currently a Base64
   ...
   `{"username": "string", "password": "string", "email": "string", "serveraddress" : "string", "auth": ""}`. Notice that: `auth` is to be left empty, the serveraddress is a domain/ip without protocol and that double quotes (instead of single ones) are required.
